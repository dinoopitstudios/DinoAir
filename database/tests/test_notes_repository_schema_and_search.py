...
def test_schema_contains_is_deleted_and_content_html(_repo, db_manager):
    ...
    assert "is_deleted" in cols, "Expected is_deleted column to be present"
    assert "content_html" in cols, "Expected content_html column to be present"
...
def test_search_with_project_filter_is_respected(repo, db_manager):
    ...
    try:
        ...
        for n in (n_proj_a, n_proj_b):
            assert repo.create_note(n, content_html=f"<p>{n.title}</p>").success
            created.append(n.id)
        ...
        assert n_proj_a.id in ids_a
        assert n_proj_b.id not in ids_a

        res_b = repo.search_notes("Alpha 123", "Title Only", project_id="proj-B")
        assert res_b.success is True
        ids_b = {nn.id for nn in res_b.data}
        assert n_proj_b.id in ids_b
        assert n_proj_a.id not in ids_b
        ...
        assert {n_proj_a.id, n_proj_b.id}.issubset(ids_all)
    finally:
...
def test_get_notes_by_tag_is_case_sensitive_membership(repo, db_manager):
    ...
    try:
        ...
        assert res_lower.success is True
        ids_lower = {nn.id for nn in res_lower.data}
        assert n1.id not in ids_lower
        ...
        assert n1.id in ids_exact
        assert n2.id not in ids_exact
    finally:
        _cleanup_ids(db_manager, created)
