...
def test_projects_crud_black_box(db_manager, projects_connection, sample_project):
    ...
    assert res["id"] == sample_project.id

    # Read
    got = db.get_project(sample_project.id)
    assert got is not None
    assert got.id == sample_project.id
    ...
    # Delete
    assert db.delete_project(sample_project.id) is True
    assert db.get_project(sample_project.id) is None
...
def test_projects_hierarchy_and_tree_black_box(db_manager, projects_connection):
    ...
    assert db.create_project(parent)["success"] is True
    assert db.create_project(child)["success"] is True
...
def test_projects_search_and_filters_black_box(db_manager, projects_connection):
    ...
    assert db.create_project(a)["success"] is True
    assert db.create_project(b)["success"] is True
    assert db.create_project(c)["success"] is True

    # Search by name/description/tags (LIKE prefilter + exact tag filter)
    results = db.search_projects("Alpha")
    assert any(p.id == "proj-a" for p in results)

    by_status = db.get_projects_by_status(ProjectStatus.ARCHIVED.value)
    assert any(p.id == "proj-c" for p in by_status)
...
def test_delete_cascade_black_box(db_manager, projects_connection):
    ...
    assert db.create_project(root)["success"] is True
    assert db.create_project(child)["success"] is True
    assert db.create_project(grandchild)["success"] is True

    # Sanity: all exist
    assert db.get_project(root.id) is not None
    assert db.get_project(child.id) is not None
    assert db.get_project(grandchild.id) is not None

    # Cascade delete
    assert db.delete_project(root.id, cascade=True) is True

    # All removed
    assert db.get_project(root.id) is None
    assert db.get_project(child.id) is None
    assert db.get_project(grandchild.id) is None
...
def test_update_rejects_invalid_parent_black_box(db_manager):
    ...
    assert db.create_project(proj)["success"] is True
...
def test_get_projects_by_status_param_matrix_black_box(db_manager, status, expected_count):
    ...
    assert db.create_project(p1)["success"] is True
    if expected_count >= 2:
        assert db.create_project(p2)["success"] is True

    assert db.create_project(other1)["success"] is True
    assert db.create_project(other2)["success"] is True
    assert db.create_project(other3)["success"] is True
