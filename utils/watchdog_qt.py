...
class WatchdogSignals(QObject):
    ...
    metrics_degraded = Signal(cast("type", SystemMetrics), str)  # Degraded metrics w/reason
...
class WatchdogThread(QThread):
    ...
    def __init__(self, config: WatchdogConfig, parent: Optional["QObject"] = None):
        ...
        self._retry_delays: dict[str, float] = {}  # Track retry delays per operation
        ...
        try:
            from .Watchdog import SystemWatchdog  # type: ignore  # pylint: disable=import-error
        except ImportError:
